    ...
    <script>
        ...
        GetData(url+"/"+newsLimit+"/"+skip, function (response){
            if(response && response.status === 200 && response.data){
                let category = response.data['sub-category'] || response.data.subcategory;
        ...
        $('#LoadMoreBtn').on('click',function (){
            ...
            GetData(url+"/"+newsLimit+"/"+skip, function (response){
                if(response && response.status === 200 && response.data){
                    let category = response.data.category || response.data['sub-category'] || response.data.subcategory;
                    let news = response.data.news || [];
                    if(news.length > 0){
                        CategoryMoreNews(news)
                    }else {
                ...
                if(i == 0){
                    DisplayTopNews.append(`
                        <div class="col-12 col-sm-12 col-md-12 col-lg-8">
                            <a style="padding-left: 0;padding-right: 0;padding-top: 0;"  href="${news[0].newsUrl || '/get-news/' + news[0].id}"  class="link  d-block card-body">
                    ...
                    $('#CategorySideNews').append(`
                        <div class="col-12 col-sm-6 col-md-6 col-lg-12">
                           <a  href="${news[1].newsUrl || '/get-news/' + news[1].id}" class="card border-0 shadow-sm d-block  pt-0 pb-0 link">
                    ...
                    $('#CategorySideNews').append(`
                         <div class="col-12 col-sm-6 mt-2 col-md-6 col-lg-12">
                            <a style="height: 280px;"  href="${news[2].newsUrl || '/get-news/' + news[2].id}" class="card border-0 shadow-sm d-block  pt-0 pb-0 link">
            ...
            for(let i = 3; i < news.length; i++){
                $('#CategoryMoreNews').append(`
                    <div class="col-12  mt-3 col-sm-6 col-md-6 col-lg-4">
                        <a style="height: 290px;" href="${news[i].newsUrl || '/get-news/' + news[i].id}" class="card shadow-sm border-0 border-bottom  pt-0 pb-0 link">
