    ...
    <div class="section-container mt-2">
        <div class="row">
            <div class="col-12 col-sm-12 col-md-7 col-lg-8 col-xl-9">
                <div class="Title pt-2 pb-2 border-bottom border-dark d-flex justify-content-between align-items-center">
    ...
    <script>
        ...
        GetData(url+"/"+newsLimit+"/"+skip, function (response){
            if(response && response.status === 200 && response.data){
                ...
                // Use category URL from API or generate from slug
                let categoryUrl = category.url || (category.slug ? `/${category.slug}` : `/category-by-id/${category.id}`);
                PillsCategory2(categoryUrl,'#SportsPills','SportsItem');
                ...
                $('#categoryContent').append(`
                    <div class="alert alert-danger" role="alert">
                      <strong>দুঃখিত! </strong> তথ্য লোড করতে সমস্যা হয়েছে
                    </div>
                `);
                BodyLoaderOFF();
            }
        ...
        $('#LoadMoreBtn').on('click',function (){
            ...
            GetData(url+"/"+newsLimit+"/"+skip, function (response){
                if(response && response.status === 200 && response.data){
                    ...
                    console.log('Load More Error:', response);
                    skip = skip-20;
                    BodyLoaderOFF();
                }
            })
        ...
        function DisplayTopNews(news){
            ...
            for(let i = 0; i < news.length; i++){
                if(i == 0){
                    let newsUrl = news[0].newsUrl || (news[0].id ? `/get-news/${news[0].id}` : '#');
                    DisplayTopNews.append(`
                        <div class="col-12 col-sm-12 col-md-12 col-lg-8">
                            <a style="padding-left: 0;padding-right: 0;padding-top: 0;"  href="${newsUrl}"  class="link  d-block card-body">
                    ...
                    let newsUrl = news[1].newsUrl || (news[1].id ? `/get-news/${news[1].id}` : '#');
                    $('#CategorySideNews').append(`
                        <div class="col-12 col-sm-6 col-md-6 col-lg-12">
                           <a  href="${newsUrl}" class="card border-0 shadow-sm d-block  pt-0 pb-0 link">
                    ...
                    let newsUrl = news[2].newsUrl || (news[2].id ? `/get-news/${news[2].id}` : '#');
                    $('#CategorySideNews').append(`
                         <div class="col-12 col-sm-6 mt-2 col-md-6 col-lg-12">
                            <a style="height: 280px;"  href="${newsUrl}" class="card border-0 shadow-sm d-block  pt-0 pb-0 link">
            ...
            for(let i = 3; i < news.length; i++){
                ...
                $('#CategoryMoreNews').append(`
                    <div class="col-12  mt-3 col-sm-6 col-md-6 col-lg-4">
                        <a style="height: 290px;" href="${newsUrl}" class="card shadow-sm border-0 border-bottom  pt-0 pb-0 link">
