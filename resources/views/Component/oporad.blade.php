<div class="mt-5">
    <div class="section-container">
        <div class="shadow-sm border-bottom border-secondary  p-2  d-flex justify-content-between align-items-center">
            ...
            <a href="/get-news-by-category/27" class="btn btn-danger rounded-pill">সকল</a>
        </div>
    </div>
</div>

<div id="international" class="section-container">
    <div class="row mt-2" id="internationalLeadNew">
        <!--
            InterNational News
        -->
    </div>
...
<script>
    ...
    function HomeInternational(){
        PillsCategory('/category-by-id/27','#InternationalPills','InternationalItem')
        ...
        $('#InternationalPills').on('click','.InternationalItem',function (){
            ...
            let LeadNews = $('#internationalLeadNew');
            if(id === "0"){
        ...
        function AllInternationalNews(){
            $('#internationalLeadNew').empty()
            GetData('/get-all-news/27/lead_news/4/0', function(response){
                if(response.status === 200){
                    let data = response.data;
                  //  let order = 5;
                    for(let i = 0; i < data.length; i++){
